...
export async function POST(
  ...
  try {
    ...
    if (isD1RuntimeEnabled()) {
      const result = await callDataService<Record<string, any>>("admin-domain/ad-automation-payment", { adminId: (session.user as { id: string }).id, userId, ...validation.data, paymentDate: new Date(validation.data.paymentDate).getTime() });
      return NextResponse.json({ success: true, message: "결제가 등록되었습니다.", data: { payment: result.payment, serviceStartDate: new Date(Number(result.serviceStartDate)), serviceEndDate: new Date(Number(result.serviceEndDate)) } });
    }
    ...
    const user = await prisma.user.findUnique({ where: { id: userId }, select: { id: true, 이름: true, adAutomationEndDate: true } });
