...
export async function POST(request: NextRequest) {
  try {
    ...
    if (isD1RuntimeEnabled()) {
      ...
      const data = await callDataService<{ workflow: Record<string, unknown> }>("admin-notifications/workflow-design-data", { adminId, workflowId });
      const workflow = data.workflow;
      if (!workflow.시안URL) return NextResponse.json({ error: "시안이 업로드되지 않았습니다." }, { status: 400 });
