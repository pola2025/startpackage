...
const STATUS_CONFIG: Record<WorkflowStatus, { label: string; color: string; description: string }> = {
  '대기': {
    label: '대기',
...
interface KanbanBoardProps {
  workflows: any[]
  onUpdateStatus: (workflowId: string, newStatus: WorkflowStatus) => Promise<void>
}
...
export function KanbanBoard({ workflows, onUpdateStatus }: KanbanBoardProps) {
  ...
  // 상태별로 워크플로우 그룹화
  const workflowsByStatus = Object.keys(STATUS_CONFIG).reduce((acc, status) => {
