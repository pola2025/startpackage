          ...
          <div className="space-y-3 md:space-y-4">
            <div className="space-y-1 md:space-y-2">
              <p className="text-navy-900 font-bold text-sm md:text-base">
                ...
                <span className="text-navy-700 font-medium text-xs md:text-sm">
                  (영업일 기준)
                </span>
              </p>
                    ...
                    workflow.type !== "홈페이지" && (
                      <Link href="/dashboard/design-threads" className="flex-1">
                        <Button
                          variant="outline"
                          size="sm"
                          className="w-full h-8 md:h-9 text-xs md:text-sm border-terra-100 text-terra-500 hover:bg-terra-50"
                        >
                                ...
                                workflow.시안URL && (
                                  <div className="flex gap-1.5 md:gap-2">
                                    <Button
                                      ...
                                      onClick={() =>
                                        window.open(workflow.시안URL!, "_blank")
                                      }
                                    >
                                      새 탭에서 보기
                                    </Button>
                                ...
                                : // 인쇄물: 발주 요청 버튼 (발주대기 상태일 때만)
                                  workflow.status === "발주대기" && (
                                    <Button
                                      size="sm"
                                      onClick={() =>
                                        handleOrderRequest(workflow.id)
                                      }
                                      disabled={loading}
