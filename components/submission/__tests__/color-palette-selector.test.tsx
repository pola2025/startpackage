...
import { ColorPaletteSelector, COLOR_PALETTE_OPTIONS } from '../color-palette-selector'
...
describe('ColorPaletteSelector 컴포넌트', () => {
  ...
  describe('렌더링', () => {
    it('제목이 표시된다', () => {
      ...
      expect(screen.getByText(/홈페이지에 사용할 색상을 선택해주세요/)).toBeInTheDocument()
    })
  ...
  describe('직접 입력', () => {
    ...
    it('showCustomInput이 false일 때 입력 필드가 숨겨진다', () => {
      render(<ColorPaletteSelector {...defaultProps} showCustomInput={false} />)

      expect(screen.queryByPlaceholderText(/네이비, 베이지/)).not.toBeInTheDocument()
    })
  })
