...
// 워크플로우 상태 매핑
const WORKFLOW_STATUS_MAP: Record<string, { label: string; color: string; step: number }> = {
  "대기": { label: "접수 대기", color: "text-gray-500", step: 0 },
  "시안중": { label: "시안 준비 중", color: "text-blue-600", step: 1 },
  "발주대기": { label: "확인 필요", color: "text-amber-600", step: 2 },
  "발주요청": { label: "제작 요청됨", color: "text-blue-600", step: 2 },
  ...
  return (
    <div className="border border-gray-200 rounded-lg overflow-hidden">
      ...
      >
        ...
        <div className="flex items-center gap-3">
          <span className={cn("text-xs font-medium px-2 py-1 rounded-full", statusColor)}>
            {statusText}
          </span>
          {statusIcon}
          {isOpen ? (
...
function WorkflowTimeline({ workflow }: { workflow: Workflow }) {
  const statusInfo = WORKFLOW_STATUS_MAP[workflow.status] || WORKFLOW_STATUS_MAP["대기"];
...
// 정보 행 컴포넌트
function InfoRow({ label, value, masked = false }: { label: string; value?: string | null; masked?: boolean }) {
...
export function MySubmissionStatus({
  ...
  const sectionStatus = useMemo(() => {
    ...
    const designInProgress = workflows.some(
      w => ["시안중", "발주대기", "발주요청"].includes(w.status)
    );
    const designComplete = workflows.some(
      w => ["발주완료", "최종확정", "제작완료", "발송완료"].includes(w.status)
    );
    ...
    return {
      ...
      docs: { complete: docsComplete, count: docsCount, total: requiredDocs.length },
      design: { complete: designComplete, inProgress: designInProgress || logoSelected || namecardSelected },
      ...
      <div className="p-4 sm:p-6 space-y-3">
        ...
        >
          <div className="bg-white rounded-lg p-4 mt-3">
            ...
            <InfoRow label="영문 브랜드명" value={submission.brandNameEnglish} />
            <InfoRow label="업종" value={submission.업종} />
            <InfoRow label="주소" value={submission.주소} />
            <button
              onClick={() => onNavigateToSection("basic")}
              className="w-full mt-3 text-sm text-blue-600 hover:text-blue-700 font-medium flex items-center justify-center gap-1"
            >
        ...
        >
          <div className="bg-white rounded-lg p-4 mt-3">
            ...
            <FileStatus label="대표자 신분증" url={submission.대표자신분증URL} />
            <FileStatus label="통신서비스 이용증명원" url={submission.통신서비스이용증명원URL} />
            <button
        ...
        <AccordionSection
          ...
          isInProgress={sectionStatus.design.inProgress && !sectionStatus.design.complete}
          defaultOpen={sectionStatus.design.inProgress && !sectionStatus.design.complete}
        >
          <div className="space-y-4 mt-3">
              ...
              {submission.로고선호스타일 || submission.로고선호색상 ? (
                <>
                  <div className="text-sm text-gray-600 space-y-1">
                    {submission.로고선호스타일 && (
                      <p>스타일: <span className="font-medium text-gray-900">{submission.로고선호스타일}</span></p>
                    )}
                    {submission.로고선호폰트 && (
                      <p>폰트: <span className="font-medium text-gray-900">{submission.로고선호폰트}</span></p>
                    )}
                    {submission.로고선호색상 && (
                      <p>색상: <span className="font-medium text-gray-900">{submission.로고선호색상}</span></p>
                    )}
                  </div>
                  {workflowsByType["로고"] && (
                ...
                <p className="text-sm text-gray-500">아직 선택하지 않았습니다</p>
              )}
              <button
            ...
            <div className="bg-white rounded-lg p-4">
              ...
              {submission.명함시안 ? (
                <>
                  <div className="text-sm text-gray-600 space-y-1">
                    <p>선택 시안: <span className="font-medium text-gray-900">{submission.명함시안}</span></p>
                    {submission.명함색상 && (
                      <p className="flex items-center gap-2">
                        ...
                        />
                        <span className="font-medium text-gray-900">{submission.명함색상}</span>
                      </p>
                    )}
                  </div>
                  {workflowsByType["명함"] && (
            ...
            {/* 기타 인쇄물 진행 상태 */}
            {(workflowsByType["명찰"] || workflowsByType["대봉투"] || workflowsByType["자문계약서"]) && (
              <div className="bg-white rounded-lg p-4">
                <div className="flex items-center gap-2 mb-3">
                  <Truck className="w-4 h-4 text-green-600" />
                  <span className="font-medium text-gray-900">기타 인쇄물 진행 현황</span>
                </div>
                <div className="space-y-3">
                  {["명찰", "대봉투", "자문계약서"].map(type => {
                    const wf = workflowsByType[type];
                    if (!wf) return null;
                    const status = WORKFLOW_STATUS_MAP[wf.status] || WORKFLOW_STATUS_MAP["대기"];
                    return (
        ...
        >
          <div className="bg-white rounded-lg p-4 mt-3">
            ...
            <InfoRow label="메타 광고관리자" value={submission.메타광고관리자값 ? "등록됨 ✓" : null} />
            <InfoRow label="Instagram ID" value={submission.InstagramID} masked />
            <InfoRow label="Gmail ID" value={submission.GmailID} masked />
            <InfoRow label="네이버검색광고 ID" value={submission.네이버검색광고ID} masked />
            <button
        ...
        >
          <div className="bg-white rounded-lg p-4 mt-3">
            ...
            {submission.홈페이지제작방식 === "외부서비스" && (
              <>
                <InfoRow label="도메인 관리 사이트" value={submission.도메인관리사이트} />
                <InfoRow label="도메인 관리 ID" value={submission.도메인관리ID} masked />
              </>
            )}
            <button
